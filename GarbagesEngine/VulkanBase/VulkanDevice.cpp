...
{	
	...
	*/
	...
	// 创建逻辑设备
		...
		VkResult result = createLogicalDevice(enabledFeatures, enabledExtensions, nullptr, false, VK_QUEUE_GRAPHICS_BIT);
	...
	*/
	uint32_t VulkanDevice::getMemoryType(uint32_t typeBits, VkMemoryPropertyFlags properties, VkBool32 *memTypeFound) const
	{
		...
		{
			spdlog::error("Could not find a matching memory type");
		}
	}
	...
	*/
	uint32_t VulkanDevice::getQueueFamilyIndex(VkQueueFlags queueFlags) const
	{
		...
		spdlog::error("Could not find a matching queue family index");
	}
